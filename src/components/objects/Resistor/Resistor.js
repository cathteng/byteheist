...
class Resistor extends Group {
    ...
    createBoxes(position, angle, material) {
        ...
        let armBody = new CANNON.Body({
            shape: new CANNON.Box(new CANNON.Vec3(armWidth, 
              armHeight, armDepth)), // use this for a finite plane
            type: CANNON.Body.STATIC,
            material: material, //
            position: new THREE.Vector3(position.x, position.y + 3.5, position.z)
          });
        ...
        if (legAngleDecision == 1) {
            newLegPos1 = new THREE.Vector3(position.x, position.y - 1.5, position.z + 12.5);
            newLegPos2 = new THREE.Vector3(position.x, position.y - 1.5, position.z - 12.5);
        } else {
            newLegPos1 = new THREE.Vector3(position.x + 12.5, position.y - 1.5, position.z);
            newLegPos2 = new THREE.Vector3(position.x - 12.5, position.y - 1.5, position.z);
        }
        let leg1Body = new CANNON.Body({
            shape: new CANNON.Box(new CANNON.Vec3(legWidth, 
              legHeight, legDepth)), // use this for a finite plane
            type: CANNON.Body.STATIC,
            material: material, //
            position: newLegPos1
          });
